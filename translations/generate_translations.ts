...
class TranslationsLoader {
  ...
  static languagesToLoad = ['en', 'zh', 'fr'];
  ...
  static async loadAllModulesAndMerge() {
    ...
    for (let lang of TranslationsLoader.languagesToLoad) {
        ...
        for (let key of Object.keys(translation[lang][module])) {
          ...
          if (key in TranslationsLoader.loadedTranslations[lang][module]) {
            if (TranslationsLoader.loadedTranslations[lang][key] == value)
              console.warn(
                'Duplicate translation key (same value)! For key:',
                key,
                'Imported by module:',
                module
              );
              ...
              console.error(
                'Duplicate translation key with different values! Fix this - Overwriting entry for key:',
                key,
                'Imported by module:',
                module
              );
      ...
      writeFileSync(
        ...
        JSON.stringify(TranslationsLoader.loadedTranslations[lang])
      );
    }
