export const zh = {
  developertools: {
    ...
    'create-app-message':
      '创建一个新的应用程序意味着为其创建一个新的永久 DID。必须仔细保存此 DID，因为这是将来升级应用程序的唯一方法。要了解如何构建应用程序，请访问https://developer.elastos.org',
    'app-name': '应用名称',
    ...
    appIdentityHelpMessage:
      '您在 Elastos DID 链上的应用程序标识符独立于任何出版物或平台，例如 Pathsport 或原生 Android / iOS。这只是证明所有权的一种方法，但也是必须执行的步骤。',
    nativeRedirectUrlHelpMessage:
      '原生应用需要将它们的 Scheme 基础 URL 保存在它们的公共 DID 文档中，以便保护应用间通信。例如：https://elastosapp.mysite.org. 重定向 URL 在移动设备上发送本地 Intent。由原生移动应用程序使用。',
    nativeCustomSchemeHelpMessage:
      '原生应用程序（Android）应该提供一个简短的自定义 Scheme（例如：myapp），例如由 trinity 原生应用程序用来发送 Intent 响应。对于 trinity 原生应用，此自定义名称必须与 trinitynative.json 中配置的名称匹配。',
    nativeCallbackUrlHelpMessage:
      '原生应用程序需要将 Intent Scheme 基础 URL 保存在其公共 DID 文档中，以确保应用程序间通信的安全。例如：https://elastosapp.mysite.org。回调 URL 将 HTTP POST 请求发送到远程 HTTP 服务器。由网站使用。',
    'help-message':
      '应用程序 DID 存储密码是用于访问创建的应用程序配置文件的普通密码。确保安全保存此密码',
    'help-message2':
      '如果您已经创建了一个应用程序，则可以使用其现有的 DID 助记词来创建另一个应用程序配置文件',
    'help-message3':
      '仅用于高级用途，仅当您要使用钱包的密码短语以增强安全性时才需要此助记词',
    'delete-app-msg':
      '为了恢复此应用程序以供将来访问和更新，您必须创建一个具有相同助记词的新应用程序',
  },
};
