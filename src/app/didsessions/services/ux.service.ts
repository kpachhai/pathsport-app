...
import {
  GlobalThemeService,
  AppTheme,
} from 'src/app/services/global.theme.service';
...
export class UXService {
  ...
  onTitleBarItemClicked(icon: TitleBarIcon) {
    ...
    switch (icon.key) {
      ...
      case 'back':
        ...
        break;
      case 'language':
        this.go('/didsessions/createidentity');
        break;
      case 'theme':
  ...
  async navigateRoot() {
    ...
    if (identities.length == 0) {
      Logger.log(
        'didsessions',
        'No existing identity. Navigating to language chooser then createidentity'
      );
      await this.nav.navigateRoot(
        App.DID_SESSIONS,
        'didsessions/createidentity',
        { animationDirection: Direction.FORWARD }
      );
    } else {
  ...
  async showOptions(ev: any, identityEntry: IdentityEntry) {
    ...
    this.options = await this.popoverCtrl.create({
      ...
      cssClass:
        this.theme.activeTheme.value == AppTheme.LIGHT
          ? 'options-component'
          : 'dark-options-component',
      componentProps: {
