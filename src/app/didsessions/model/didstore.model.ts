...
export class DIDStore {
  ...
  public static async create(didStoreId = null): Promise<DIDStore> {
    ...
    if (!didStoreId) didStoreId = Util.uuid(6, 16);

    Logger.log(
      'didsessions',
      'Initializing a new DID Store with ID ' + didStoreId
    );
    try {
  ...
  public async addDID(
    ...
    await did.addCountryCredential(newIdentity.country, storePassword);
    ...
    return new Promise((resolve, reject) => {
      didManager.initDidStore(
        ...
        (payload: string, memo: string) => {
          ...
          Logger.warn(
            'didsessions',
            'Create ID transaction callback called but we do not handle it!'
          );
        },
        (pluginDidStore: DIDPlugin.DIDStore) => {
          Logger.log(
            'DIDSessions',
            'Initialized DID Store is ',
            pluginDidStore
          );
   ...
   */
  public async createPrivateIdentity(
    mnemonicPass: string,
    storePass: string,
    mnemonicLang: DIDPlugin.MnemonicLanguage,
    mnemonic: string
  ): Promise<boolean> {
    // Create a private root key
    Logger.log('didsessions', 'Creating private root key');
    await this.initPluginPrivateIdentity(
      mnemonicLang,
      mnemonic,
      mnemonicPass,
      storePass,
  ...
  private initPluginPrivateIdentity(
    language,
    mnemonic,
    mnemonicPass,
    storePass,
  ...
  public async loadAll() {
    ...
    try {
      ...
      Logger.error(
        'didsessions',
        'Fatal error while loading from DID Store id.',
        e
      );
      return;
    }
