...
export class DID {
  ...
  async addCredential(
    ...
    return new Promise(async (resolve, reject) => {
      Logger.log(
        'didsessions',
        'Adding credential with id:',
        credentialId,
        props,
        userTypes
      );

      let types: String[] = ['SelfProclaimedCredential'];
      ...
      try {
        Logger.log(
          'didsessions',
          'Asking DIDService to create the credential with id ' + credentialId
        );
        // the max validity days is 5*365 (5 years).
        credential = await this.createPluginCredential(
          credentialId,
          types,
          5 * 365,
          props,
  ...
  private createPluginCredential(
    credentialId: DIDURL,
    type,
    validityDays,
    properties,
  ...
  private addPluginCredential(
    credential: DIDPlugin.VerifiableCredential
  ): Promise<void> {
    Logger.log(
      'didsessions',
      'DIDService - storeCredential',
      this.getDIDString(),
      JSON.parse(JSON.stringify(credential))
    );
    return new Promise(async (resolve, reject) => {
  ...
  getNameCredentialValue(): string {
    for (let credential of this.credentials) {
      if (
        new DIDURL(credential.pluginVerifiableCredential.getId()).matches(
          '#name'
        )
      ) {
  ...
  getAvatarCredentialValue(): CredentialAvatar {
    for (let credential of this.credentials) {
      if (
        ...
        let subject = credential.pluginVerifiableCredential.getSubject();
        if (subject.avatar) return subject.avatar as CredentialAvatar;
        else return null;
      }
    }
    return null;
  ...
  /**
   * Retrieves the "#country" verifiable credential value in this did, if any.
   */
  getLocationCredentialValue(): string {
    for (let credential of this.credentials) {
      if (
        new DIDURL(credential.pluginVerifiableCredential.getId()).matches(
          '#country'
