...
export class NotificationsPage implements OnInit {
  ...
  async start(notification: LauncherNotification) {
    ...
    if (notification.type == LauncherNotificationType.TIP) {
      ...
      this.events.publish(
        'notifications.tip',
        this.tipsService.findTipByIdentifier(tipData.key)
      );
    } else {
      switch (notification.app) {
        case App.CONTACTS:
          ...
          break;
        case App.PSPROFILE:
          void this.globalNav.navigateTo(App.PSPROFILE, '/psprofile/friends');
          break;
        ...
        case App.CRPROPOSAL_VOTING:
          void this.globalNav.navigateTo(
            App.CRPROPOSAL_VOTING,
            '/crproposalvoting/proposals/ALL'
          );
          break;
        case App.SCANNER:
        ...
        default:
          Logger.log(
            'Launcher',
            'Notifications.page.start - No routing available'
          );
      }
    }
  }
  ...
  getNotificationIcon(notification: LauncherNotification) {
    if (notification.type === LauncherNotificationType.SYSTEM) {
      ...
      if (
        notification.contactAvatar &&
        Object.keys(notification.contactAvatar).length !== 0
      ) {
        return (
          'data:' +
          notification.contactAvatar.contentType +
          ';base64,' +
          notification.contactAvatar.base64ImageData
        );
      } else {
  ...
  getNotificationHeader(notification: LauncherNotification) {
    if (notification.type === LauncherNotificationType.CONTACT) {
      if (notification.contactName) return notification.contactName;
      else return this.translate.instant('launcher.from-unknown-contact');
    } else if (notification.app) {
      return this.translate.instant(
        this.notificationService.getAppTitle(notification.app)
      );
  ...
  getNotificationMessage(notification: LauncherNotification) {
    if (notification.type == LauncherNotificationType.TIP) {
      ...
      return (
        translatedMessage.replace(/<\/?[^>]+>/gi, '').substr(0, 100) + '...'
      );
    } else {
