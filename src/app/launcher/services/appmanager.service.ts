...
export class AppmanagerService {
  ...
  public init() {
    ...
    this.intentSubscription = this.globalIntentService.intentListener.subscribe(
      (receivedIntent) => {
    ...
    this.tipSubscription = this.events.subscribe(
      'notifications.tip',
      (notification) => {
        ...
        apps: [
          ...
          // },
          {
            ...
            description: this.translate.instant(
              'launcher.app-identity-description'
            ),
          ...
          },
          {
            ...
            description: this.translate.instant(
              'launcher.app-contacts-description'
            ),
      ...
      },
      {
        ...
        apps: [
          ...
          // },
          {
            ...
            description: this.translate.instant(
              'launcher.app-settings-description'
            ),
  ...
  async getVisit() {
    let visit = await this.storage.getSetting<boolean>(
      this.didService.signedIdentity.didString,
      'launcher',
      'visit',
      false
    );
  ...
  async startApp(app: RunnableApp): Promise<void> {
    ...
    else if (app.startCall) app.startCall();
    else
      Logger.error(
        'launcher',
        'Failed to start app without either routerPath or startCall entry point.',
        app
      );
  }
  ...
  async toggleNotifications() {
    ...
    if (!this.notificationsShown)
      // Notifications are currently not shown so we want to show them
  ...
  private async dismissAllModals() {
    ...
    while ((modalElement = await this.modalController.getTop())) {
      ...
      if (newModalElement && newModalElement === modalElement) {
        // just in case
