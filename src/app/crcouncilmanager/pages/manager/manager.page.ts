...
export class CRCouncilManagerPage implements OnInit {
  ...
  async ionViewWillEnter() {
    this.titleBar.setTitle(
      this.translate.instant('crcouncilmanager.registration-manager')
    );
    ...
    let rpcApiUrl = this.globalElastosAPIService.getApiUrl(
      ElastosApiUrlType.ELA_RPC
    );
    Logger.log(App.CRCOUNCIL_MANAGER, 'rpcApiUrl:', rpcApiUrl);
    const result = await this.jsonRPCService.httpPost(rpcApiUrl, param);
    let did = GlobalDIDSessionsService.signedInDIDString.replace(
      'did:elastos:',
      ''
    );
  ...
  async registration() {
    await this.voteService.selectWalletAndNavTo(
      App.CRCOUNCIL_MANAGER,
      '/crcouncilmanager/registration'
    );
  }
