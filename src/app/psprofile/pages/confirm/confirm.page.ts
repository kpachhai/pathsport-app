...
export class ConfirmPage implements OnInit {
  ...
  async addContact() {
    ...
    this.zone.run(() => {
      if (contactAlreadyAdded) {
        void this.globalNav.navigateRoot(App.PSPROFILE, '/psprofile/friends');
      } else {
        if (!this.name) {
          this.friendsService.showCustomization(
            this.friendsService.pendingContact,
            true
          );
        } else {
          void this.globalNav.navigateRoot(
            App.PSPROFILE,
            '/psprofile/friends/' + this.id
          );
        }
      }
    });
  ...
  async denyContact() {
    if (this.friendsService.contactNotifierInviationId) {
      Logger.log(
        'contacts',
        'Rejecting contact notifier invitation',
        this.friendsService.contactNotifierInviationId
      );
      await this.contactNotifier.rejectInvitation(
        this.friendsService.contactNotifierInviationId
      );
      ...
      Logger.log(
        'contacts',
        'Rejected contact did not come from a "viewfriendinvitation" intent'
      );
    }
