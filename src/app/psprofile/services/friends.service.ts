...
import {
  ...
  Contact as ContactNotifierContact,
} from 'src/app/services/contactnotifier.service';
import {
...
export class FriendsService extends GlobalService {
  ...
  async checkFirstVisitOperations() {
    let visit = await this.storage.getSetting<boolean>(
  ...
  async getStoredContacts(): Promise<Contact[]> {
    Logger.log(
      'contacts',
      'Getting stored contacts for DID ',
      GlobalDIDSessionsService.signedInDIDString
    );
    let contacts = await this.storage.getSetting(
      GlobalDIDSessionsService.signedInDIDString,
      'contacts',
      'contacts',
      []
  ...
  private async checkContacts(): Promise<void> {
    if (!this.contactsChecked) {
      ...
      for (let contact of this.contacts) {
        ...
        contact.id !== 'did:elastos'
          ? await this.resolveDIDDocument(contact.id, true)
          : null;
      }
    }
  }
  ...
  getContactNotifierContacts() {
    void this.contactNotifier
      .getAllContacts()
      .then(async (notifierContacts) => {
        Logger.log('Contacts', 'Found all Notifier Contacts', notifierContacts);
        notifierContacts.forEach((notifierContact) => {
          const alreadyAddedContact = this.contacts.find(
            (contact) => contact.id === notifierContact.getDID()
          );
          if (!alreadyAddedContact) {
            ...
            const newContact: Contact = {
              ...
              credentials: {
                ...
                avatar:
                  contactAvatar &&
                  Object.getOwnPropertyNames(contactAvatar).length !== 0
                    ? Avatar.fromContactNotifierContactAvatar(contactAvatar)
                    : null,
              },
              avatarLocal: null,
              customName: null,
            ...
            Logger.log(
              ...
              alreadyAddedContact + ' is already added'
            );
          }
        });
  ...
  async scanDID() {
    let res = await this.globalIntentService.sendIntent(
      'https://scanner.elastos.net/scanqrcode'
    );
    ...
    if (!scannedContentHandled) {
      void this.native.genericToast(
        this.translate.instant('contacts.failed-read-scan')
      );
    }
  }
  ...
  async addContactByIntent(did: string, carrierAddress?: string) {
    Logger.log('psprofile', 'Received contact by intent', did, carrierAddress);

    if (this.didService.getUserDID() === did) {
      void this.native.genericToast('contacts.please-dont-add-self');
      void this.globalNav.navigateRoot('contacts', '/psprofile/friends');
    } else {
      const targetContact: Contact = this.contacts.find(
        (contact) => contact.id === did
      );
      if (targetContact) {
        ...
        if (carrierAddress) {
          this.contacts[
            this.contacts.indexOf(targetContact)
          ].notificationsCarrierAddress = carrierAddress;
          await this.storage.setSetting(
            GlobalDIDSessionsService.signedInDIDString,
            'contacts',
            'contacts',
          ...
          void this.globalNav.navigateRoot(
            'contacts',
            '/psprofile/friends/' + targetContact.id
          );
          void this.native.genericToast(
            promptName + this.translate.instant('contacts.did-carrier-added')
          );
          Logger.log(
            'contacts',
            'Contact is already added but carrier address is updated',
            this.contacts[this.contacts.indexOf(targetContact)]
          );
        } else {
          void this.native.genericToast(
            promptName + this.translate.instant('contacts.is-already-added')
          );
          void this.globalNav.navigateRoot(
            'contacts',
            '/psprofile/friends/' + targetContact.id
  ...
  resolveDIDDocument(
    ...
    Logger.log(
      'Contacts',
      'Resolving DID document for DID string ',
      didString,
    ...
    return new Promise((resolve, reject) => {
      ...
      didManager.resolveDidDocument(
        didString,
        true,
        async (didDocument: DIDPlugin.DIDDocument) => {
          Logger.log(
            'Contacts',
            'DIDDocument resolved for DID ' + didString,
            didDocument
          );
          if (didDocument && !updatingFriends) {
            await this.buildPublishedContact(
              didDocument,
              carrierAddress,
            ...
            void this.native.genericToast(
              this.translate.instant('contacts.contact-is-unpublished')
            );
            await this.buildUnpublishedContact(
              didString,
              carrierAddress,
              requiresConfirmation
            );
      ...
      if (contact.id === newDoc.id.didString) {
        ...
        for (let key of newDoc.verifiableCredential) {
          ...
          if ('avatar' in key.credentialSubject) {
            contact.credentials.avatar = await Avatar.fromAvatarCredential(
              key.credentialSubject.avatar
            );
          }
  ...
  async buildUnpublishedContact(
    ...
    Logger.log(
      'contacts',
      'Building contact using unresolved DID for confirm-prompt',
      didString
    );
  ...
  async buildPublishedContact(
    ...
    Logger.log(
      'contacts',
      'Building contact using resolved DID document for confirm-prompt',
      resolvedDidDocument
    );
    ...
    for (let key of resolvedDidDocument.verifiableCredential) {
      ...
      if ('avatar' in key.credentialSubject) {
        ...
        this.pendingContact.credentials.avatar =
          await Avatar.fromAvatarCredential(key.credentialSubject.avatar);
      }
      if ('nickname' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has nickname');
        this.pendingContact.credentials.nickname =
          key.credentialSubject.nickname;
      }
      if ('gender' in key.credentialSubject) {
      ...
      if ('birthDate' in key.credentialSubject) {
        ...
        this.pendingContact.credentials.birthDate =
          key.credentialSubject.birthDate;
      }
      if ('birthPlace' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has birth place');
        this.pendingContact.credentials.birthPlace =
          key.credentialSubject.birthPlace;
      }
      if ('occupation' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has occupation');
        this.pendingContact.credentials.occupation =
          key.credentialSubject.occupation;
      }
      if ('education' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has education');
        this.pendingContact.credentials.education =
          key.credentialSubject.education;
      }
      if ('telephone' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has telephone');
        this.pendingContact.credentials.telephone =
          key.credentialSubject.telephone;
      }
      if ('email' in key.credentialSubject) {
      ...
      if ('interests' in key.credentialSubject) {
        ...
        this.pendingContact.credentials.interests =
          key.credentialSubject.interests;
      }
      if ('description' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has description');
        this.pendingContact.credentials.description =
          key.credentialSubject.description;
      }
      if ('url' in key.credentialSubject) {
      ...
      if ('facebook' in key.credentialSubject) {
        ...
        this.pendingContact.credentials.facebook =
          key.credentialSubject.facebook;
      }
      if ('instagram' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has instagram');
        this.pendingContact.credentials.instagram =
          key.credentialSubject.instagram;
      }
      if ('snapchat' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has snapchat');
        this.pendingContact.credentials.snapchat =
          key.credentialSubject.snapchat;
      }
      if ('telegram' in key.credentialSubject) {
        Logger.log('contacts', 'Resolved DID has telegram');
        this.pendingContact.credentials.telegram =
          key.credentialSubject.telegram;
      }
      if ('wechat' in key.credentialSubject) {
      ...
      if ('elaAddress' in key.credentialSubject) {
        ...
        this.pendingContact.credentials.elaAddress =
          key.credentialSubject.elaAddress;
      }
    }
  ...
  showConfirmPrompt(isPublished: boolean) {
    ...
    const props: NavigationExtras = {
      queryParams: {
        ...
        image: this.pendingContact.credentials.avatar
          ? JSON.stringify(this.pendingContact.credentials.avatar)
          : null, // Temporary BPI fix to avoid receiving [Object object] in the confirm screen, but better avoid using query params for potentially large data like avatars. Need to fix here @chad.
        isPublished: isPublished,
      },
    };
    void this.globalNav.navigateRoot('contacts', '/psprofile/confirm', props);
  }
  ...
  addContact(): Promise<boolean> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const targetContact: Contact = this.contacts.find(
        (contact) => contact.id === this.pendingContact.id
      );

      if (targetContact) {
        if (this.pendingContact.carrierAddress) {
          this.contacts[this.contacts.indexOf(targetContact)].carrierAddress =
            this.pendingContact.carrierAddress;
        ...
        if (this.pendingContact.notificationsCarrierAddress) {
          Logger.log(
            'contacts',
            'Sending friend invitation through contact notifier'
          );
          ...
          Logger.log(
            'contacts',
            'Added friend has no associated contact notifier carrier address'
          );
        }
        ...
        if (this.contactNotifierInviationId) {
          Logger.log(
            'contacts',
            'Accepting contact notifier invitation',
            this.contactNotifierInviationId
          );
          void this.contactNotifier.acceptInvitation(
            this.contactNotifierInviationId
          );
          ...
          Logger.log(
            'contacts',
            'Confirmed contact did not come from a "viewfriendinvitation" intent'
          );
        }
        ...
        void this.native.genericToast(
          promptName + this.translate.instant('contacts.was-added')
        );
        resolve(false);
      }
  ...
  public safeAddContact(contact: Contact) {
    if (this.contacts.find((c) => c.id === contact.id)) {
      Logger.warn(
        'contacts',
        'Trying to add contact that already exists in the list! Logic error',
        contact,
        this.contacts
      );
      return;
  ...
  updateNotifierContact(contact: Contact) {
    void this.contactNotifier
      ...
      .then((notifierContact: ContactNotifierContact) => {
        if (notifierContact) {
          ...
          if (targetAvatar) {
            Logger.log(
              'contacts',
              'Updating notifier contact avatar' + contact.id
            );
          ...
          if (targetName) {
            Logger.log(
              'contacts',
              'Updating notifier contact name' + contact.id
            );
  ...
  async deleteContact(contact: Contact) {
    ...
    Logger.log(
      'contacts',
      'Deleting contact from the contact notifier database'
    );
    ...
    Logger.log(
      'contacts',
      'Updated contacts after deleting:' + contact.credentials.name,
      this.contacts
    );
    await this.saveContactsState();
    ...
    void this.native.genericToast(
      promptName + this.translate.instant('contacts.was-deleted')
    );
    void this.globalNav.navigateRoot('contacts', '/psprofile/friends');
  }
  ...
  async customizeContact(
    ...
    for (let contact of this.contacts) {
      if (contact.id === id) {
        Logger.log(
          'Contacts',
          "Updating contact's custom values" +
            customName +
            customNote +
            customAvatar
        );
    ...
    void this.globalNav.navigateRoot(App.PSPROFILE, '/psprofile/friends/' + id);
  }
  ...
  viewContact(didString: string) {
    void this.getStoredContacts().then(async (contacts: Contact[]) => {
      const targetContact = contacts.find(
        (contact) => contact.id === didString
      );
      if (targetContact) {
        void this.globalNav.navigateTo(
          'contacts',
          '/psprofile/friends/' + didString
        );
      } else {
  ...
  getContacts(isSingleInvite: boolean, intent: string) {
    void this.getStoredContacts().then((contacts: Contact[]) => {
      Logger.log(
        'contacts',
        'Fetched stored contacts for pickfriend intent',
        contacts
      );
      const realContacts = contacts.filter(
        (contact) => contact.id !== 'did:elastos'
      );
      if (realContacts.length > 0) {
        ...
        void this.globalNav.navigateTo('contacts', '/psprofile/invite', props);
      } else {
        void this.globalNav.navigateRoot('contacts', '/psprofile/friends');
  ...
  getFilteredContacts(isSingleInvite: boolean, ret) {
    void this.getStoredContacts().then((contacts: Contact[]) => {
      ...
      if (realContacts.length > 0) {
        ...
        Logger.log(
          'contacts',
          'Intent requesting friends with credential',
          ret.params.filter.credentialType
        );
        ...
        if (this.filteredContacts.length > 0) {
          ...
          void this.globalNav.navigateRoot('friends', '/psprofile/friends');
          void this.native.alertNoContacts(
            'pickfriend',
            this.managerService.handledIntentId,
  ...
  async sendRemoteNotificationToContact(
    contactId: string,
    title: string,
    url: string
  ) {
    let contactNotifierContact = await this.contactNotifier.resolveContact(
      contactId
    );
    if (contactNotifierContact) {
      Logger.log(
        'contacts',
        'Sending shared content to friend with DID ' + contactId
      );
      ...
      Logger.warn(
        'contacts',
        'Not sending shared content to friend with DID ' +
          contactId +
          ' because he is not in the contact notifier'
      );
    }
  }
  ...
  async shareToContacts(isFilter: boolean) {
    ...
    if (!isFilter) {
      await Promise.all(
        this.contacts.map(async (contact) => {
          if (contact.isPicked) {
            await this.sendRemoteNotificationToContact(
              contact.id,
              this.shareIntentData.title,
              this.shareIntentData.url
            );
      ...
      await Promise.all(
        this.filteredContacts.map(async (contact) => {
          if (contact.isPicked) {
            await this.sendRemoteNotificationToContact(
              contact.id,
              this.shareIntentData.title,
    ...
    Logger.log(
      'contacts',
      'Tried to send ' + sentNotificationsCount + ' notifications to friends'
    );
  ...
  inviteContacts(isFilter: boolean, intent: string) {
    ...
    if (!isFilter) {
      ...
      contactsForIntent = this.filteredContacts.filter(
        (contact) => contact.isPicked
      );
  ...
  sendIntentRes(contacts: Contact[], intent: string) {
    if (contacts.length > 0) {
      ...
      void this.native.genericToast(
        this.translate.instant('contacts.select-before-invite')
      );
    }
  }
  ...
  sortContacts() {
    ...
    this.contacts.map((contact) => {
      ...
      if (
        (!contact.credentials.name &&
          contact.customName &&
          contact.customName === 'Anonymous Contact' &&
          !this.letters.includes('Anonymous')) ||
        (!contact.credentials.name &&
          !contact.customName &&
          !this.letters.includes('Anonymous'))
      ) {
      ...
      if (
        contact.credentials.name &&
        !contact.customName &&
        !this.letters.includes(contact.credentials.name[0].toUpperCase())
      ) {
      ...
      if (
        (!contact.credentials.name &&
          contact.customName &&
          contact.customName !== 'Anonymous Contact' &&
          !this.letters.includes(contact.customName[0].toUpperCase())) ||
        (contact.credentials.name &&
          contact.customName &&
          contact.customName !== 'Anonymous Contact' &&
          !this.letters.includes(contact.customName[0].toUpperCase()))
    ...
    this.letters.push(
      this.letters.splice(this.letters.indexOf('Anonymous'), 1)[0]
    );
  }
