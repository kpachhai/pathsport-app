...
export class GlobalElastosAPIService extends GlobalService {
  ...
  public activeProvider: BehaviorSubject<ElastosAPIProvider> =
  ...
  async onUserSignIn(signedInIdentity: IdentityEntry): Promise<void> {
    this.languageSubscription = this.language.activeLanguage.subscribe(
      (lang) => {
        // For translation.
        this.initProvidersList();
      }
    ...
    if (!provider) {
      ...
      Logger.log(
        'elastosapi',
        'Current provider name could not be found, looking for the default one'
      );
    ...
    Logger.log(
      'elastosapi',
      "User's Elastos API provider is:",
  ...
  public getApiUrl(type: ElastosApiUrlType): string {
    ...
    if (!(activeNetworkTemplate in activeProvider.endpoints)) {
      Logger.warn(
        'elastosapi',
        'Unknown network template ' + activeNetworkTemplate + '!'
      );
      return null;
    }
  ...
  public async autoDetectTheBestProvider(): Promise<void> {
    Logger.log(
      'elastosapi',
      'Trying to auto detect the best elastos api provider'
    );
  ...
  private async findTheBestProvider(): Promise<ElastosAPIProvider> {
    ...
    let testPromises: Promise<void>[] = this.availableProviders.map((p) =>
      this.callTestAPIOnProvider(p)
    );
  ...
  private callTestAPIOnProvider(provider: ElastosAPIProvider): Promise<void> {
    ...
    return new Promise(async (resolve) => {
      let testApiUrl =
        provider.endpoints[`${environment.network}`].mainChainRPC;
      ...
      try {
        ...
        Logger.log(
          ...
          'Provider ' +
            provider.name +
            ' just answered the test api call with value (block height) ',
          data
        );
        ...
        Logger.warn(
          'elastosapi',
          'Auto detect api call to ' + testApiUrl + ' failed with error:',
          e
        );
  ...
  private async setResolverUrl(): Promise<void> {
    ...
    Logger.log(
      'elastosapi',
      'Changing DID plugin resolver in DID and Hive plugins to :',
      didResolverUrl
    );
    // DID Plugin
    await new Promise<void>((resolve, reject) => {
      didManager.setResolverUrl(
        ...
        (err) => {
          Logger.error(
            'DIDSessionsService',
            'didplugin setResolverUrl error:',
            err
          );
          reject(err);
        }
