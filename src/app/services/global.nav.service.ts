...
export class GlobalNavService {
  ...
  public navigateRoot(
    ...
    Logger.log(
      'Nav',
      'Setting ' + context + ' navigation context root to: ' + route
    );
  ...
  public navigateDIDSessionHome(): Promise<boolean> {
    ...
    return this.navCtrl.navigateRoot(didSessionHome.route, {
  ...
  public async exitCurrentContext(navigate = true): Promise<void> {
    ...
    if (!currentStep) {
      Logger.error(
        'Nav',
        "Can't get the currentStep, this.navigationHistory:",
        this.navigationHistory
      );
      return;
    }
    ...
    while (this.canGoBack()) {
      ...
      if (currentStep.context == startContext) this.navigationHistory.pop();
      // Same context, unstack the step
      else {
        if (navigate) {
          // Found the previous context, back to there.
          await this.navCtrl.navigateBack(
            currentStep.route,
            currentStep.routerOptions
          );
        }
        return;
      }
