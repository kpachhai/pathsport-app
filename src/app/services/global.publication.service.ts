...
const assistAPIKey =
  'IdSFtQosmCwCB9NOLltkZrFy5VqtQn8QbxBKQoHPw7zp3w0hDOyOYjgL53DO3MDH';
...
namespace AssistPublishing {
  ...
  export class AssistPublisher extends DIDPublisher {
    ...
    public async publishDID(
      ...
      Logger.log(
        'publicationservice',
        'Requesting identity publication to Assist',
        didString
      );

      if (typeof payloadObject === 'string')
        throw new Error(
          'Payload must be a JSON object, not a stringified JSON'
        );
      ...
      this.manager.persistentInfo.did.publicationStatus =
        DIDPublicationStatus.NO_ON_GOING_PUBLICATION;
      await this.manager.savePersistentInfoAndEmitStatus(
        this.manager.persistentInfo
      );
      ...
      const requestBody = {
        did: didString,
        memo: memo || '',
        requestFrom: 'Pathsport',
        didRequest: payloadObject,
      };

      Logger.log(
        'publicationservice',
        'Assist API request body:',
        requestBody,
      ...
      try {
        ...
        if (
          ...
          Logger.log(
            'publicationservice',
            'All good, DID has been submitted. Now waiting.'
          );

          this.manager.persistentInfo.did.publicationStatus =
            DIDPublicationStatus.AWAITING_PUBLICATION_CONFIRMATION;
          ...
          let error =
            "Successful response received from the assist API, but response can't be understood";
          throw error;
        }
      } catch (err) {
        Logger.error('publicationservice', 'Assist publish api error:', err);
        this.manager.persistentInfo.did.publicationStatus =
          DIDPublicationStatus.FAILED_TO_PUBLISH;
        await this.manager.savePersistentInfoAndEmitStatus(
          this.manager.persistentInfo
        );
      }
    ...
    private async getAssistEndpoint(): Promise<string> {
      ...
      switch (activeNetworkTemplate) {
        ...
        default:
          throw new Error(
            'Assist service cannot be used to published on network ' +
              activeNetworkTemplate
          );
      }
    }
            ...
            if (
              ...
              if (
                ...
                if (response.data.blockchainTxId)
                  this.manager.persistentInfo.did.assist.txId =
                    response.data.blockchainTxId;
              } else if (
                response.data.status == AssistTransactionStatus.QUARANTINED
              ) {
                ...
                response.data.status == AssistTransactionStatus.COMPLETED
              ) {
                this.manager.persistentInfo.did.publicationStatus =
                  DIDPublicationStatus.PUBLISHED_AND_CONFIRMED;
                await this.manager.savePersistentInfoAndEmitStatus(
                  this.manager.persistentInfo
                );
              } else {
...
namespace WalletPublishing {
  export class WalletPublisher extends DIDPublisher {
    ...
    public async publishDID(
      ...
      Logger.log(
        'publicationservice',
        'Sending didtransaction intent with params:',
        params
      );
      ...
      try {
        ...
        Logger.log(
          'publicationservice',
          'Got didtransaction intent response from the wallet.',
          response
        );
        ...
        if (response.result && response.result.txid) {
          Logger.log(
            'publicationservice',
            'didtransaction response.result.txid ',
          ...
          Logger.log(
            'publicationservice',
            'didtransaction response.result.txid is null'
          );
          this.manager.persistentInfo.did.publicationStatus =
            DIDPublicationStatus.FAILED_TO_PUBLISH;
          await this.manager.savePersistentInfoAndEmitStatus(
        ...
        Logger.error(
          'publicationservice',
          'Failed to send app manager didtransaction intent!',
          err
        );
        this.manager.persistentInfo.did.publicationStatus =
          DIDPublicationStatus.FAILED_TO_PUBLISH;
    ...
    private async checkPublicationStatusAndUpdate(): Promise<void> {
      ...
      if (
        nowSec - this.manager.persistentInfo.did.wallet.publicationTime >
        PUBLICATION_AWAIT_TIMEOUT_SEC
      ) {
        Logger.log(
          'publicationservice',
          "New DID can't be resolved after several seconds. Failing publication."
        );
        this.manager.persistentInfo.did.publicationStatus =
          DIDPublicationStatus.FAILED_TO_PUBLISH;
        await this.manager.savePersistentInfoAndEmitStatus(
      ...
      return new Promise(async (resolve, reject) => {
        Logger.log(
          'publicationservice',
          'Checking transaction status on chain for txid ' +
            this.manager.persistentInfo.did.wallet.txId
        );
        ...
        void this.jsonRPC
          .httpPost(endpoint, request)
          .then(async (response: EIDChainResolveResponse) => {
            Logger.log(
              'publicationservice',
              'EID chain successful resolve response:',
              response
            );
            if (
              response &&
              ...
              Logger.log(
                'publicationservice',
                'we got a clear status from the EID resolving RPC api.'
              );

              if (
                '0x' + response.transaction[0].txid ===
                this.manager.persistentInfo.did.wallet.txId
              ) {
                Logger.log(
                  'publicationservice',
                  'All good, we got the published txid in the resolved document (latest)'
                );
                this.manager.persistentInfo.did.publicationStatus =
                  DIDPublicationStatus.PUBLISHED_AND_CONFIRMED;
                await this.manager.savePersistentInfoAndEmitStatus(
...
class DIDPublishingManager {
  ...
  public emitPublicationStatusChangeFromPersistentInfo() {
    ...
    if (this.persistentInfo.did.assist && this.persistentInfo.did.assist.txId)
      ...
      this.persistentInfo.did.wallet &&
      this.persistentInfo.did.wallet.txId
    )
  ...
  private async getPublishingMedium(): Promise<string> {
    if (!GlobalDIDSessionsService.signedInDIDString) return 'assist'; // No signed in user? We may be in a DID creation flow. Anyway, use assist in this case.
  ...
  public async publishDIDFromRequest(
    ...
    return this.activePublisher.publishDID(
      didString,
      payloadObject,
      memo,
      showBlockingLoader
...
export class GlobalPublicationService {
   ...
   */
  public publishDIDFromRequest(
    didString: string,
    payloadObject: JSONObject,
    memo: string,
    showBlockingLoader = false
  ): Promise<void> {
    return this.manager.publishDIDFromRequest(
      didString,
      payloadObject,
      memo,
      showBlockingLoader
  ...
  public publishDIDFromStore(
    ...
    return new Promise<void>(async (resolve, reject) => {
      // eslint-disable-next-line @typescript-eslint/no-misused-promises, no-async-promise-executor
      const didStore = await this.openDidStore(
        storeId,
        async (payload: string, memo: string) => {
          // Callback called by the DID SDK when trying to publish a DID.
          Logger.log(
            'publicationservice',
            'Create ID transaction callback is being called',
            payload,
            memo
          );
          ...
          await this.publishDIDFromRequest(
            didString,
            payloadAsJson,
            memo,
            showBlockingLoader
      ...
      const localDIDDocument = await this.loadLocalDIDDocument(
        didStore,
        didString
      );
      if (localDIDDocument) {
        ...
        localDIDDocument.publish(
          ...
          (err) => {
            ...
            Logger.log(
              'publicationservice',
              'Local DID Document publish(): error',
              err
            );
            reject(err);
          }
        ...
        reject(
          'Failed to load DID document for DID ' +
            didString +
            ' in store id ' +
            didStore.getId()
        );
