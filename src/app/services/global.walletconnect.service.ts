...
import {
  ...
  IdentityEntry,
} from './global.didsessions.service';
import {
  JsonRpcRequest,
...
export class GlobalWalletConnectService extends GlobalService {
  ...
  public walletConnectSessionsStatus = new BehaviorSubject<
    Map<string, WalletConnect>
  >(new Map());
      ...
      if (receivedIntent.action === 'rawurl') {
        if (receivedIntent.params && receivedIntent.params.url) {
          ...
          if (this.canHandleUri(rawUrl)) {
            this.zone.run(() => {
              void this.handleWCURIRequest(
                rawUrl,
                WalletConnectSessionRequestSource.EXTERNAL_INTENT
              );
            });
          }
        }
  ...
  public onUserSignIn(signedInIdentity: IdentityEntry): Promise<void> {
    ...
    this.activeWalletSubscription =
      this.walletManager.activeMasterWallet.subscribe(
        async (activeWalletId) => {
          if (activeWalletId) {
              ...
              if (c.connected) {
                try {
                  c.updateSession({
                    chainId: c.chainId,
                    accounts: [
                      await this.getAccountFromMasterWallet(
                        this.walletManager.getMasterWallet(activeWalletId)
                      ),
                    ],
                  });
                } catch (e) {
                  Logger.warn(
                    'walletconnect',
                    'Non critical updateSession() error:',
                    e
                  );
                }
              }
  ...
  private getAccountFromMasterWallet(wallet: MasterWallet): Promise<string> {
    let subwallet = wallet.getSubWallet(
      StandardCoinName.ETHSC
    ) as ETHChainSubWallet; // TODO: ONLY ELASTOS ETH FOR NOW
  ...
  public handleWCURIRequest(
    ...
    this.zone.run(() => {
      void this.nav.navigateTo(
        'walletconnectsession',
        '/settings/walletconnect/preparetoconnect',
        {}
      );
    });
  ...
  private prepareConnectorForEvents(connector: WalletConnect) {
    ...
    connector.on('disconnect', (error, payload) => {
      Logger.log(
        'walletconnect',
        'Receiving disconnection request',
        error,
        payload
      );
      ...
      if (this.shouldShowDisconnectionInfo(payload))
        this.native.genericToast(
          'settings.wallet-connect-session-disconnected'
        );
   ...
   */
  private shouldShowDisconnectionInfo(disconnectionPayload: {
    event: string;
    ...
    if (
      ...
      !disconnectionPayload.params ||
      disconnectionPayload.params.length == 0
    )
      return true;
  ...
  public async killAllSessions(): Promise<void> {
    ...
    Logger.log(
      'walletconnect',
      'Killing ' + sessions.length + ' sessions from persistent storage',
      sessions
    );
    ...
    for (let session of sessions) {
      ...
      try {
        ...
        Logger.warn(
          'walletconnect',
          'Error while killing WC session',
          connector,
          e
        );
      }
  ...
  */
  private handleSessionRequest(
    ...
    void this.zone.run(async () => {
      ...
      // User UI prompt
      await this.nav.navigateTo(
        'walletconnectsession',
        '/settings/walletconnect/connect',
        {
  ...
  */
  private async handleCallRequest(
    ...
    if (request.method === 'essentials_url_intent') {
      ...
      try {
        ...
        Logger.log(
          'walletconnect',
          'Got esctransaction intent response',
          response
        );
        ...
        if (response && response.result.status === 'published') {
          ...
          connector.rejectRequest({
            ...
            error: {
              code: -1,
              message:
                'Errored or cancelled - TODO: improve this error handler',
            },
          });
        }
      } catch (e) {
  ...
  private async handleAddERCTokenRequest(
    connector: WalletConnect,
    request: JsonRpcRequest
  ) {
    ...
    let params =
      request.params[0] instanceof Array ? request.params[0] : request.params;
    let response: {
      action: string;
      result: {
        added: boolean;
      ...
      'https://wallet.elastos.net/adderctoken',
      params
    );
   ...
   */
  private async handleEssentialsCustomRequest(
    connector: WalletConnect,
    request: JsonRpcRequest
  ) {
    ...
    try {
      Logger.log(
        'walletconnect',
        'Sending custom essentials intent request',
        intentUrl
      );
      let response = await this.intent.sendUrlIntent(intentUrl);
      Logger.log(
        'walletconnect',
        'Got custom request intent response',
        response
      );
      ...
      if (
        await this.prefs.developerModeEnabled(
  ...
  public async acceptSessionRequest(
    connectorKey: string,
    ...
    switch (activeNetworkTemplate) {
      ...
      default:
        throw new Error(
          'Network currently selected in settings is not supported with wallet connect yet (unknown chain id). To be improved.'
        );
    }

    Logger.log(
      'walletconnect',
      'Accepting session request with params:',
      connectorKey,
      ethAccountAddresses,
      chainId
    );
  ...
  public async rejectSession(connectorKey: string, reason: string) {
    Logger.log(
      'walletconnect',
      'Rejecting session request',
      this.initiatingConnector
    );
    ...
    if (connectorKey) {
      ...
      Logger.log(
        'walletconnect',
        'Rejecting session with connector key',
        connectorKey,
        connector
      );
    } else {
    ...
    if (connector) {
      ...
      if (connector.connected) {
        try {
          ...
          Logger.warn(
            'walletconnect',
            'Reject session exception (disconnect):',
            e
          );
        }
      } else {
  ...
  private async restoreSessions() {
    ...
    Logger.log(
      'walletconnect',
      'Restoring ' + sessions.length + ' sessions from persistent storage',
      sessions
    );
    for (let session of sessions) {
      let connector = new WalletConnect({
  ...
  private async loadSessions(): Promise<WalletConnectSession[]> {
    Logger.log(
      'walletconnect',
      'Loading storage sessions for user ',
      GlobalDIDSessionsService.signedInDIDString
    );
    let sessions = await this.storage.getSetting<WalletConnectSession[]>(
      GlobalDIDSessionsService.signedInDIDString,
      'walletconnect',
      'sessions',
      []
  ...
  private async saveSession(session: WalletConnectSession) {
    ...
    if (existingSessionIndex < 0) sessions.push(session);
    // add new
    else sessions[existingSessionIndex] = session; // replace
  ...
  private async deleteSession(session: WalletConnectSession) {
    ...
    let existingSessionIndex = sessions.findIndex((s) => s.key === session.key);
    if (existingSessionIndex >= 0) sessions.splice(existingSessionIndex, 1);
