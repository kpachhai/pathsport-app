...
export type IdentityEntry = {
  ...
  name: string;
  /** Identity entry country, set by the user */
  country?: string;
...
export class GlobalDIDSessionsService {
  ...
  public async init(): Promise<void> {
    ...
    this.identities = await this.storage.getSetting<IdentityEntry[]>(
      null,
      'didsessions',
      'identities',
      []
    );
    let lastSignedInIdentity = await this.storage.getSetting<IdentityEntry>(
      null,
      'didsessions',
      'signedinidentity',
      null
    ...
    if (lastSignedInIdentity) {
      let identity = this.identities.find(
        (entry) => lastSignedInIdentity.didString == entry.didString
      );
      if (identity) {
  ...
  public async addIdentityEntry(entry: IdentityEntry): Promise<void> {
    ...
    if (
      this.signedInIdentity &&
      this.signedInIdentity.didString === entry.didString
    ) {
  ...
  public async signIn(
    ...
    if (entry.didStoragePath == null) {
      ...
      entry.didStoragePath = await internalManager.getDidStoragePath(
        entry.didStoreId,
        entry.didString
      );
      await this.saveDidSessionsToDisk();
    ...
    GlobalDIDSessionsService.signedInDIDString =
      this.signedInIdentity.didString;

    await GlobalServiceManager.getInstance().emitUserSignIn(
      this.signedInIdentity
    );
  ...
  public navigateHome(): Promise<boolean> {
    if (!this.signedInIdentity)
      throw new Error(
        'DID Sessions cannot navigate to essentials home screen as there is no user signed in yet'
      );
  ...
  public activeIdentityWasBackedUp(): Promise<boolean> {
    return this.storage.getSetting(
      this.getSignedInIdentity().didString,
      'didsessions',
      'identitybackedup',
      false
    );
