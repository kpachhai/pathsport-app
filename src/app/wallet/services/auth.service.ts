...
export class AuthService {
  ...
  public async createAndSaveWalletPassword(walletId: string): Promise<string> {
    ...
    let oldPassword = (await passwordManager.getPasswordInfo(
      passwordKey
    )) as PasswordManagerPlugin.GenericPasswordInfo;
    if (oldPassword) {
      // In case of user click 'createMasterwallet' too quickly.
    ...
    if (result.value) {
      // Main Password was created and wallet password could be saved
      return password;
    } else {
  ...
  public async getWalletPassword(
    walletId: string,
  ...
  public deleteWalletPassword(walletId: string): Promise<string> {
    return new Promise<string>(async (resolve, reject) => {
      try {
        const resultInfo = (await passwordManager.deletePasswordInfo(
          'wallet-' + walletId
        )) as PasswordManagerPlugin.BooleanWithReason;
        if (resultInfo) {
          if (resultInfo.value) {
            resolve(null);
          } else {
